...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --max-instances=5 \
  --no-cpu-throttling \
  --set-env-vars "ENV=prod,KAFKA_CONSUMER_ENABLED=true,KAFKA_BOOTSTRAP_SERVERS=10.20.3.3:9092,JWT_ISSUER=https://auth.travelhub.app,JWT_AUDIENCE=travelhub-api,APP_URL=https://app.travelhub.app,SUPPORT_EMAIL=soporte@travelhub.app,SENDGRID_FROM_EMAIL=noreply@apitravelhub.site,SENDGRID_FROM_NAME=TravelHub,SENDGRID_SANDBOX=false,FCM_PROJECT_ID=${PROJECT_ID}" \
